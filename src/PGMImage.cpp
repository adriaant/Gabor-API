		...
		else								// ASCII
		{
			...
			for ( i = 0; i < mHeight; i++ )
			{
				for ( j = 0; j < mWidth; j++ )
				{
					int pix;
					imgFile >> pix;
					mPixels[i][j] = (unsigned char)( pix / pow( 2, mNumBits-8 ) );
				}
			}
		}
	}
		...
		if ( mMagicNumber[1] == '4' )		// RAWBITs
		{
			...
			for ( i = 0; i < mHeight; i++ )
				for ( j = 0; j < mWidth; j += 8 )
				{
					...
					for ( int k = 0; k < 8; k++ )
					{
						unsigned int y = x/( (unsigned int)pow( 2,7-k ) );
						if ( y )
							mPixels[i][j+k] = 0;
						else
							mPixels[i][j+k] = 255;
						x -= y*( (unsigned int)pow( 2,7-k ) );
					}
				}
		}
		else
...
void PGMImage::Write( char* filename, float*** pixels, int height, int width )
{
	...
	if ( mVerbosity ) cerr << " image to file \"" << filename << "\"...";
